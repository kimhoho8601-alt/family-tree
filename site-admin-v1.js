(() => {
  ...
  const ADMIN_GATE_CODE = '71028601';
  const SERVER_SAVE_CODE = 'Geno-74K9-Q2M6';
  const state = {
  ...
  function ensureGate() {
    let dialog = document.getElementById('siteAdminGate');
    if (dialog) return dialog;
    dialog = document.createElement('dialog');
    dialog.id = 'siteAdminGate';
    dialog.innerHTML = `<form class="site-gate" id="siteAdminGateForm"><div class="gate-icon">G</div><h2>관리자 확인</h2><p>공지 편집 및 접속 현황은 관리자만 확인할 수 있습니다.</p><input id="siteGateCode" type="password" inputmode="numeric" autocomplete="off" placeholder="관리자 코드 입력" aria-label="관리자 코드"><div id="siteGateError" class="site-gate-error"></div><div class="site-gate-actions"><button type="button" class="ghost" id="siteGateCancel">취소</button><button type="submit" class="primary">확인</button></div></form>`;
    document.body.appendChild(dialog);
    dialog.querySelector('#siteGateCancel').addEventListener('click', () => dialog.close());
    dialog.addEventListener('click', (e) => { if (e.target === dialog) dialog.close(); });
    dialog.querySelector('#siteAdminGateForm').addEventListener('submit', (e) => {
      e.preventDefault();
      const input = dialog.querySelector('#siteGateCode');
      const error = dialog.querySelector('#siteGateError');
      if (input.value !== ADMIN_GATE_CODE) {
  ...
  function ensureDialog() {
    ...
    dialog.querySelector('#siteNoticeSave').addEventListener('click', async () => {
      ...
      try {
        const data = await callApi({ action: 'saveNotice', adminCode: SERVER_SAVE_CODE, title: title.value, body: body.value, isActive: active.checked });
  ...
  async function openAdmin() {
    ...
    fillDialog(dialog);
    const status = dialog.querySelector('#siteAdminStatus');
    status.textContent = '접속 현황을 불러오는 중...';
    status.className = 'site-admin-status';
    ...
    try {
      ...
      status.textContent = '접속 현황을 불러오지 못했습니다.';
      status.className = 'site-admin-status error';
    }
  }
