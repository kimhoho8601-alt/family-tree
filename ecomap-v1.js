(() => {
  ...
  function clippedPoints(system){const c=state.center,dx=system.x-c.x,dy=system.y-c.y,len=Math.max(1,Math.hypot(dx,dy)),ux=dx/len,uy=dy/len;return{a:{x:c.x+ux*86,y:c.y+uy*86},b:{x:system.x-ux*82,y:system.y-uy*42}};}
  ...
  function nodeMarkup(system){const note=String(system.note||'').trim(),lines=note?[note.slice(0,20),note.slice(20,40)].filter(Boolean):['더블클릭해 메모 입력'];return`<g class="eco-node system ${state.selected===system.id?'selected':''}" data-eco-node="${system.id}" transform="translate(${system.x} ${system.y})"><rect x="-86" y="-45" width="172" height="90" rx="11" fill="#fff" stroke="#5f5256" stroke-width="3"/>${textMarkup(system.name,-13)}<line x1="-70" y1="5" x2="70" y2="5" class="eco-table-divider" stroke="#e5dadd"/><text class="eco-resource-note" y="22" fill="#786b6f" text-anchor="middle" font-size="9">${lines.map((line,index)=>`<tspan x="0" dy="${index?14:0}">${esc(line)}</tspan>`).join('')}</text></g>`;}
  function render(){
    ...
    const system=selected(),strength=q('#ecoRelationStrength'),direction=q('#ecoRelationDirection'),del=q('#ecoDeleteSystem');
    [strength,direction,del].forEach(el=>{if(el)el.disabled=!system;});
    q('#ecoSelectedSystemLabel').textContent=system?`${system.name} 연결 설정`:'환경체계를 선택하세요';
    if(system){strength.value=system.strength;direction.value=system.direction;}
    q('#ecoCenterName').value=state.center.name;
    q('#ecoSystemList').innerHTML=state.systems.length?state.systems.map(item=>`<button type="button" class="eco-list-item ${state.selected===item.id?'active':''}" data-eco-list="${item.id}"><i></i><span><b>${esc(item.name)}</b><small>${esc(item.note||'메모 없음')}</small></span><span>${strengthNames[item.strength]}</span></button>`).join(''):'<div class="eco-list-item"><i></i><span><b>등록된 환경체계가 없습니다</b><small>위 입력창이나 빠른 추가를 사용하세요</small></span></div>';
  }
  function choose(id){state.selected=id;render();save();}
  function addSystem(name){name=String(name||'').trim();if(!name)return;const count=state.systems.length,angle=-Math.PI/2+count*(Math.PI*2/Math.max(6,count+1)),radius=count<8?245:305;const item={id:makeId(),name,strength:'normal',direction:'both',note:'',x:state.center.x+Math.cos(angle)*radius,y:state.center.y+Math.sin(angle)*radius};state.systems.push(item);state.selected=item.id;save();render();}
  ...
  q('#ecoSystemForm').addEventListener('submit',event=>{event.preventDefault();addSystem(q('#ecoSystemName').value);q('#ecoSystemName').value='';q('#ecoSystemName').focus();});
  document.querySelectorAll('[data-eco-preset]').forEach(button=>button.addEventListener('click',()=>{const name=button.dataset.ecoPreset;if(state.systems.some(item=>item.name===name)){choose(state.systems.find(item=>item.name===name).id);return;}addSystem(name);}));
