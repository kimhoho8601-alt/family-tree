(() => {
  ...
  document.documentElement.dataset.familyCore = 'v9';
  window.__FAMILY_CORE_VERSION = 'v9';
  ...
  function ensureParentTargetOptions() {
    ...
    ['father', 'mother'].forEach(kind => {
      ...
      cards.forEach((card, index) => {
        ...
        const touched = card.dataset.v9TargetTouched === 'true';
  ...
  function build() {
    ...
    state.layoutVersion = 'v9';
    save();
    render();
    activatePanel('editPanel');

    requestAnimationFrame(() => {
      const dom = state.people.map(p => ({name:p.name, expected:[p.x,p.y], rendered:els.nodes.querySelector(`[data-id="${p.id}"]`)?.getAttribute('transform') || ''}));
      window.__LAST_FAMILY_LAYOUT = dom;
      document.documentElement.dataset.familyLayoutCheck = dom.every(x => x.rendered.includes(`translate(${x.expected[0]} ${x.expected[1]})`)) ? 'ok' : 'mismatch';
    });
  ...
  form.addEventListener('change', e => {
    if (e.target.matches('.aq-parent-target')) e.target.closest('.aq-parent').dataset.v9TargetTouched = 'true';
  }, true);
